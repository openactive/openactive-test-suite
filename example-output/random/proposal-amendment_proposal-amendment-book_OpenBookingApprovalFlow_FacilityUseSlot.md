...
# proposal-amendment >> proposal-amendment-book (OpenBookingApprovalFlow >> FacilityUseSlot)

**Booking Flow:** OpenBookingApprovalFlow

**Opportunity Type:** FacilityUseSlot

**Feature:** Approval Flow / Proposal Amendment - Amend an opportunity using an OrderProposal (Implemented) 

**Test:**  Successful booking using the Booking Flow with Approval, creating an amendment

A successful end to end booking, via Booking Flow with Approval, of an opportunity, creating an amendment.

### Running only this test

```bash
npm start -- --runInBand test/features/approval/proposal-amendment/implemented/proposal-amendment-book-test.js
```

---

⚠️ 18 passed with 0 failures, 97 warnings and 10 suggestions 
...
```json
{
  ...
  "test:testOpportunityCriteria": "https://openactive.io/test-interface#TestOpportunityBookableWithNegotiation",
...
---
Response status code: 200 OK. Responded in 384.90965ms.
```json
{
  "@context": "https://openactive.io/",
  "@type": "Slot",
  "@id": "https://localhost:5001/api/identifiers/facility-uses/1239/facility-use-slots/12390"
}
```

### Opportunity Feed extract for OrderItem 0 Request
GET http://localhost:3000/opportunity/https%3A%2F%2Flocalhost%3A5001%2Fapi%2Fidentifiers%2Ffacility-uses%2F1239%2Ffacility-use-slots%2F12390?useCacheIfAvailable=true


---
Response status code: 200 OK. Responded in 427.083655ms.
```json
{
  "data": {
    "@context": [
    ...
    "facilityUse": {
      "@type": "FacilityUse",
      "@id": "https://localhost:5001/api/identifiers/facility-uses/1239",
    ...
    "offers": [
      {
        "@type": "Offer",
        "@id": "https://localhost:5001/api/identifiers/facility-uses/1239/facility-use-slots/12390#/offers/0",
    ...
    "remainingUses": 6,
    "startDate": "2021-07-07T20:48:10+00:00",
    "endDate": "2021-07-07T23:58:10+00:00"
  }
}
```
### Specs
...
* ✅ matches the criteria 'TestOpportunityBookableWithNegotiation' required for this test
...
### C1 Request
PUT https://localhost:5001/api/openbooking/order-quote-templates/e2772028-2a44-4ad7-9acd-131f99250911
  ...
  "orderedItem": [
    {
      "@type": "OrderItem",
      "position": 0,
      "acceptedOffer": "https://localhost:5001/api/identifiers/facility-uses/1239/facility-use-slots/12390#/offers/0",
      "orderedItem": "https://localhost:5001/api/identifiers/facility-uses/1239/facility-use-slots/12390"
    }
  ],
  "payment": {
    "@type": "Payment",
...
---
Response status code: 200 OK. Responded in 98.454849ms.
```json
{
  "@context": "https://openactive.io/",
  "@type": "OrderQuote",
  "@id": "https://localhost:5001/api/openbooking/order-quotes/e2772028-2a44-4ad7-9acd-131f99250911",
  "lease": {
    "@type": "Lease",
  ...
  "orderedItem": [
    {
      ...
      "orderedItem": {
        ...
        "facilityUse": {
          ...
          "url": "https://example.com/events/1239"
        },
        "maximumUses": 6,
        "remainingUses": 6,
        "startDate": "2021-07-07T20:48:10+00:00",
...
### C2 Request
PUT https://localhost:5001/api/openbooking/order-quotes/e2772028-2a44-4ad7-9acd-131f99250911
...
```json
{
  ...
  "customer": {
    "@type": "Person",
    "email": "Hulda.Botsford@gmail.com",
    "telephone": "1-443-896-8000 x2438",
    "givenName": "Kshlerin",
    "familyName": "Astrid",
    "identifier": "39188956-3089-48ba-80e4-7f817d586d78"
  },
  "orderedItem": [
    {
      "@type": "OrderItem",
...
---
Response status code: 200 OK. Responded in 126.251089ms.
```json
{
  "@context": "https://openactive.io/",
  "@type": "OrderQuote",
...
### P Request
PUT https://localhost:5001/api/openbooking/order-proposals/e2772028-2a44-4ad7-9acd-131f99250911
...
---
Response status code: 201 Created. Responded in 129.834728ms.
```json
{
  "@context": "https://openactive.io/",
  "@type": "OrderProposal",
  "@id": "https://localhost:5001/api/openbooking/order-proposals/e2772028-2a44-4ad7-9acd-131f99250911",
  ...
  "orderedItem": [
    {
      "@type": "OrderItem",
      "@id": "https://localhost:5001/api/openbooking/orders/e2772028-2a44-4ad7-9acd-131f99250911#/orderedItems/62",
      "acceptedOffer": {
        "@type": "Offer",
        "@id": "https://localhost:5001/api/identifiers/facility-uses/1239/facility-use-slots/12390#/offers/0",
        "allowCustomerCancellationFullRefund": true,
  ...
  "orderProposalVersion": "https://localhost:5001/api/openbooking/order-proposals/e2772028-2a44-4ad7-9acd-131f99250911/versions/f99b15a5-af92-40b1-8e9a-c5c3a4456711",
  "payment": {
    "@type": "Payment",
    "identifier": "lPsLYSFHc",
    "name": "AcmeBroker Points",
...
## Simulate Seller Proposal Amendment (Test Interface Action)

### Orders (order-proposals) Feed listen for &#x27;e2772028-2a44-4ad7-9acd-131f99250911&#x27; change (auth: primary) Request
POST http://localhost:3000/order-listeners/order-proposals/primary/e2772028-2a44-4ad7-9acd-131f99250911


---
Response status code: 200 OK. Responded in 341.748851ms.
```json
{
  "headers": {
  ...
  "message": "Listening for UUID: 'e2772028-2a44-4ad7-9acd-131f99250911' in feed: order-proposals, for Booking Partner: primary from startingFeedPage using headers"
}
```

### Call TestInterface Action of type: test:SellerAmendOrderProposalSimulateAction Request
...
---
Response status code: 204 No Content. Responded in 152.938922ms.
### Specs
* ✅ should return 204 on success

## Order Feed Update (after Simulate Seller Amendment)

### Orders (order-proposals) Feed collect for &#x27;e2772028-2a44-4ad7-9acd-131f99250911&#x27; change (auth: primary) Request
GET http://localhost:3000/order-listeners/order-proposals/primary/e2772028-2a44-4ad7-9acd-131f99250911


---
Response status code: 200 OK. Responded in 2816.371934ms.
```json
{
  "state": "updated",
  "kind": "Order",
  "id": "e2772028-2a44-4ad7-9acd-131f99250911",
  "modified": 637607614504939600,
  "data": {
    "@context": "https://openactive.io/",
    "@type": "OrderProposal",
    "@id": "https://localhost:5001/api/openbooking/order-proposals/e2772028-2a44-4ad7-9acd-131f99250911",
    "identifier": "e2772028-2a44-4ad7-9acd-131f99250911",
    "orderProposalStatus": "https://openactive.io/AwaitingSellerConfirmation",
    "orderedItem": [
      {
        "@type": "OrderItem",
    ...
    "orderProposalVersion": "https://localhost:5001/api/openbooking/order-proposals/e2772028-2a44-4ad7-9acd-131f99250911/versions/5e39c272-1696-4dad-87d6-09af75fdf4c0",
    "totalPaymentDue": {
      "@type": "PriceSpecification",
      "openBookingPrepayment": "https://openactive.io/Required",
      "price": 3.13,
...
### Specs
* ✅ should return 200 on success
* ✅ should have orderProposalStatus: AwaitingSellerConfirmation
* ✅ should have a different orderProposalVersion to that returned by P (as amendment has occurred)

## Order Feed Update (after Simulate Seller Amendment) >> validation of OrderFeed
### Specs
* ✅ passes validation checks
...
---
Response status code: 200 OK. Responded in 274.302047ms.
```json
{
  "headers": {
    "Accept": "application/json, application/vnd.openactive.booking+json; version=1",
...
---
Response status code: 204 No Content. Responded in 57.576667ms.
### Specs
* ✅ should return 204 on success

## Order Feed Update (after Simulate Seller Approval)
...
---
Response status code: 200 OK. Responded in 3538.206403ms.
```json
{
  "state": "updated",
  "kind": "Order",
  "id": "e2772028-2a44-4ad7-9acd-131f99250911",
  "modified": 637607614537357600,
  "data": {
    "@context": "https://openactive.io/",
    "@type": "OrderProposal",
    "@id": "https://localhost:5001/api/openbooking/order-proposals/e2772028-2a44-4ad7-9acd-131f99250911",
...
### Specs
* ✅ should return 200 on success
* ✅ should have orderProposalStatus: SellerAccepted
* ✅ should have orderProposalVersion same as that returned on the feed after Proposal Amendment

## Order Feed Update (after Simulate Seller Approval) >> validation of OrderFeed
### Specs
* ✅ passes validation checks
...
### B Request
PUT https://localhost:5001/api/openbooking/orders/e2772028-2a44-4ad7-9acd-131f99250911
...
---
Response status code: 500 Internal Server Error. Responded in 77.964307ms.
```json
{
  "@context": "https://openactive.io/",
...
---
Response status code: 201 Created. Responded in 103.095995ms.
```json
{
  "@context": [
    "https://openactive.io/",
  ...
  "@type": "Order",
  "@id": "https://localhost:5001/api/openbooking/orders/e2772028-2a44-4ad7-9acd-131f99250911",
  "identifier": "e2772028-2a44-4ad7-9acd-131f99250911",
  "bookingService": {
    "@type": "BookingService",
    "name": "Acme booking system",
...
```
### Specs
* ✅ should return a response containing `"@type": "OrderProposalVersionOutdatedError"` with status code `500`
* ✅ should return 201 on success
...
### Validations
 * ⚠️ $.description: Recommended property `description` is missing from `OrderProposalVersionOutdatedError`.
 * ⚠️ $.instance: Recommended property `instance` is missing from `OrderProposalVersionOutdatedError`.
