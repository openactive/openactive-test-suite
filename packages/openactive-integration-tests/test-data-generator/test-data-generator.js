...
const { DefaultMap, TallyMap } = require('../test/helpers/map-utils');
...
(async () => {
  ...
  /** { [sellerCriteria] => { [criteria] => [numberOfOpportunitiesRequired] } } */
  const criteriaRequirementsBySellerCriteria = Object.entries(criteriaRequirementsJson.criteriaRequirements).reduce(
    (result, [featureIdentifier, featuresCriteriaRequirementsBySellerCriteria]) => {
    ...
    /** @type {DefaultMap<string, TallyMap<string>>} */(new DefaultMap(() => new TallyMap())),
  );
