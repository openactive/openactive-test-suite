...
FeatureHelper.describeFeature(module, {
  ...
  testName: 'Successful booking and unsuccessful cancellation due to PatchNotAllowedOnProperty error',
...
function (configuration, orderItemCriteria, featureIsImplemented, logger) {
  ...
  describe('Orders Feed', function () {
    ...
    it('Order Cancellation should have PatchNotAllowedOnProperty as @type with expected description', function () {
      expect(state.uResponse.body['@type']).to.equal('PatchNotAllowedOnPropertyError');
      expect(state.uResponse.body.description).to.equal("Only 'https://openactive.io/CustomerCancelled' is permitted for this property.");
    });
  });
});
