...
FeatureHelper.describeFeature(module, {
  ...
  testIdentifier: 'opportunity-attendance-update-customer-attended',
  testName: "Changes to an opportunity's attendance (via AttendeeAttendedSimulateAction) should update the Order Feed.",
  testDescription: "After B, invoke an `AttendeeAttendedSimulateAction`. This should create an update in the Order Feed with the OrderItem's orderItemStatus changed to `https://openactive.io/CustomerAttended`",
  // The primary opportunity criteria to use for the primary OrderItem under test
  testOpportunityCriteria: 'TestOpportunityBookable',
  controlOpportunityCriteria: 'TestOpportunityBookable',
  testInterfaceActions: ['test:AttendeeAttendedSimulateAction'],
},
TestRecipes.simulateActionAndExpectOrderFeedUpdateAfterSimpleC1C2Book({ actionType: 'test:AttendeeAttendedSimulateAction' },
  ({ orderFeedUpdate, orderItemCriteriaList }) => {
    it('OrderItems should have CustomerAttended statuses', () => {
      // new = after the AttendeeAttendedSimulateAction was invoked
      ...
      for (const newOrderItem of newOrderItems) {
        ...
        expect(orderItemStatus).to.be.equal('https://openactive.io/AttendeeAttended');
      }
    });
  }));
