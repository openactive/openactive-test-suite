...
(configuration, orderItemCriteriaList, featureIsImplemented, logger) => {
  ...
  const { fetchOpportunities, c1, assertOpportunityCapacityAfterC1, c2, assertOpportunityCapacityAfterC2, bookRecipe } = FlowStageRecipes.initialiseSimpleC1C2BookFlow(orderItemCriteriaList, logger);

  // ## Set up tests
  FlowStageUtils.describeRunAndCheckIsSuccessfulAndValid(fetchOpportunities);
  FlowStageUtils.describeRunAndCheckIsSuccessfulAndValid(c1);
  FlowStageUtils.describeRunAndCheckIsSuccessfulAndValid(assertOpportunityCapacityAfterC1);
  FlowStageUtils.describeRunAndCheckIsSuccessfulAndValid(c2);
  FlowStageUtils.describeRunAndCheckIsSuccessfulAndValid(assertOpportunityCapacityAfterC2);
